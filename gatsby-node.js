...
const createJsonMarkdownPropertyNodes = ({
  ...
  const resultObj = Object.keys(treeNode).filter(key => !blacklist.includes(key)).reduce((_resultObj, key) => {
    if (typeof treeNode[key] !== "object") {
      const propNodeIdInput = `${transformerJsonNode.id} >>> ${_TYPE_JSON_REMARK_PROPERTY} >>> ${objectPath.concat(`.${key}`)}`;
      ...
      if (!nodeExists) {
        ...
        propNode.internal.contentDigest = createContentDigest(JSON.stringify(propNode)); // sets object reference on new tree via closure.
        // TODO: refactor
        ...
        const treeNodePromise = createNode(propNode).then(result => {
          ...
          _resultObj.treeNewState.treeNode[key]["originalValue"] = treeNode[key];
          return result;
        });
        createParentChildLink({
      ...
      const newGatsbyType = gatsbyType.concat(isNum(key) ? `` // it's an array, don't concat the key
      : upperFirst(key));
      ...
      _resultObj.promises = _resultObj.promises.concat(...__resultObj.promises);
      return _resultObj;
    }
  }, {
    promises: [],
...
const isFileAllowed = ({
  ...
  return !pathsExclude.includes(fileNode.dir) && !pathsExclude.includes(fileNode.absolutePath) && (pathsInclude.includes(fileNode.dir) || pathsInclude.includes(fileNode.absolutePath));
};
...
exports.onCreateNode = async (nodeApiArgs, pluginOptions = {}) => {
  ...
  if (!jsonParent || node.internal.owner !== _OWNER_JSON_TRANSFORMER || !isFileAllowed({
    fileNode: jsonParent,
    pathsInclude: paths,
    pathsExclude
  }) // || node.path !== "/about"
  ...
  return Promise.all(resultObj.promises).then(() => {
    ...
    propAggregateNode.internal.contentDigest = createContentDigest(JSON.stringify(propAggregateNode));
